...
CHANNELS = 3
IMAGE = "Geralt.jpg"  # Choose your cover image (PNG)
STEGO_IMAGE = "stego_image.png"  # Choose name for your saving stego file
EMBEDDED_IMAGE = "BLue Square2.jpg"
emb_img = Image.open(EMBEDDED_IMAGE)
emb_img = np.array(emb_img, dtype=np.float64)
emb_img = np.array(emb_img, dtype=np.uint8)

f = open(EMBEDDED_IMAGE, 'rb')
a = f.read()
image = np.asarray(bytearray(a))
f.close()
print(str(image)[1:-1].replace('\n', ' ').replace('  ', ' ').replace('  ', ' '))
SECRET_MESSAGE = str(image)[1:-1].replace('\n', ' ').replace('  ', ' ').replace('  ', ' ')
...
def embed_encoded_data_into_DCT(encoded_bits, dct_blocks):
    ...
    encoded_data_len = bitstring.pack('uint:32', len(encoded_bits))  # Pack – это менеджер геометрии, который размещает виджеты по горизонтали и вертикали
    converted_blocks = []
